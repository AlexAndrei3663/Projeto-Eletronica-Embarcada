...
# 389 "./mcc_generated_files/pin_manager.h"
...
void comm (void){
    ...
    txByte[0] = 0xBF & ((uint8_t)motor<<4 | (0b10000000) | (pos_atual & 0x03));
    txByte[1] = 0x7F & (((uint8_t)position)>>1);
    txByte[2] = 0x7F & (((uint8_t)speed)<<2);
    txByte[3] = 0x7F & ((temp)<<1);
...
void deslocamento(){
    ...
    }else if (pos_final > pos_atual & PORTAbits.RA7==1) {
        PWM3_LoadDutyValue(409);
    }else if(pos_final < pos_atual & PORTAbits.RA7==0){
        PWM3_LoadDutyValue(409);
...
void main(void){
 ...
 IOCBF0_SetInterruptHandler(S1);
    IOCBF3_SetInterruptHandler(S2);

    while (1){
        temp = (int)(ADC_GetConversion(TEMP)/5.0);
        speed = (int)(.83/(((float)colect_value)*.000002))*4;
        position = .83*pulsos;
        if(pos_final != pos_atual){
            controle();
        }
